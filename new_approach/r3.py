...
interfaces = {
    #   node: (listen,      send)
    'd': ('10.10.7.2', '10.10.7.1')
}

recieved_interfaces = {
    #   node: (listen,      send)
    's': ('10.10.3.2', '10.10.3.1'),
    'r2': ('10.10.6.2', '10.10.6.1')
}
...
def listen_interface(other_endpoint, interface):
    ...
    if other_endpoint is 's':
        avg_rtt = sock.recv(buffer_size)  # receive avg_rtt values from s and d
        avg_rtt = float(avg_rtt.decode())
        file_lock.acquire()
        with open('link_costs.txt', 'a') as file:
            file.write(other_endpoint + "-" + endpoint + "=" + avg_rtt)  # s-r3=avg_rtt
        file_lock.release()
...
def link_interface(other_endpoint, interface):
    ...
    file_lock.acquire()
    with open('link_costs.txt', 'a') as file:
        file.write(other_endpoint + "-" + endpoint + "=" + avg_rtt)  # d-r3=avg_rtt
    file_lock.release()
